...
function drawHeader(doc: jsPDF, pw: number, ml: number, mr: number) {
  ...
  doc.setCharSpace(0)
  doc.text('RSG Integrated Operations Centre', ml+10, 8.5)
...
function drawFooters(doc: jsPDF, pw: number, ph: number, ml: number, mr: number) {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    doc.setCharSpace(0)
    doc.text('RSG Integrated Operations Centre  —  Vizzio  ·  April 2026  ·  Confidential', ml, ph-5.5)
    doc.setFont('Inter','bold')
...
export async function generateFullPDF() {
  ...
  doc.setTextColor(...C.ink)
  doc.setCharSpace(0)
  doc.text('RSG Integrated Operations Centre', ml, 75)
  doc.setFont('Inter','normal')
  doc.setFontSize(16)
  doc.setTextColor(...C.muted)
  ...
  doc.setFontSize(10)
  doc.setTextColor(...C.muted)
  doc.text('Version 1.0  ·  April 2026  ·  Vizzio', ml, 97)
  doc.setDrawColor(...C.rule)
  doc.setLineWidth(0.3)
