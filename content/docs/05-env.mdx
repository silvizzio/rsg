...
On MONTHLY and YEARLY, each KPI sits on a tinted background that reads the direction of its movement: green when the change is the one you want, red when it is not. The tint is judged by outcome, not by whether the figure rose or fell, so a drop in electricity reads green and a rise in renewable share reads green. NOW carries no tint, because there is no prior window to move against.
